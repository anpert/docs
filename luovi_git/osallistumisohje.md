...
Tervetuloa mukaan! Tämä repo noudattaa **PR-virtausta**: älä puske suoraan `main`-haaraan, vaan tee työsi erillisessä (siis ei main-) haarassa ja avaa **Pull Request** (PR).
...
* (voit tarkistaa, että se ei ole jo repo: anna komento __git status__. Jos se on jo, hyppää seuraavan kohdan yli) 
* anna koodi: 
```bash
git init -b main
```
* (voit käydä kopioimassa etärepon osoitteen. Se on 
