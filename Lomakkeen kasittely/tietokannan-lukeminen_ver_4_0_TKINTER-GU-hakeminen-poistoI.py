...
class DatabaseManager:
    ...
    # DatabaseManager- luokka ja sen metodit connect etc.
    ...
    def update_row(self, table, columns, values, where_col, where_val):
        """
        Päivittää yhden rivin turvallisesti
        """
    ...
    def delete_row(self, table, where_col, where_val):
        """
        Poistaa rivin
        """
        query = f"DELETE FROM {table} WHERE {where_col}=?"
    ...
    def get_primary_key(self, table):
        """
        Hakee taulun pääavaimen (PRIMARY KEY)
        """
        cursor = self.conn.cursor()
...
class App:
    ...
    def delete_selected(self):
        ...
        if not selected:
            messagebox.showwarning("Huom", "Valitse rivi ensin")
            return

        values = self.tree.item(selected[0])["values"]
        columns = list(self.current_df.columns)

        pk = self.db.get_primary_key(self.current_table)

        if pk is None:
            messagebox.showerror("Virhe", "Taululla ei ole pääavainta.")
            return
        ...
        # varmistus
        confirm = messagebox.askyesno("Vahvista", "Poistetaanko rivi?")
        if not confirm:
            return

        try:
            self.db.delete_row(self.current_table, pk, pk_value)
    ...
    def edit_row(self, event):
        ...
        def save():
            ...
            pk_index = list(columns).index(pk)
            pk_value = values[pk_index]
